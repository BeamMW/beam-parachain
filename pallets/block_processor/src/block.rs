...
fn test_pow_deserialization() {
    ...
    assert_eq!(expected_solution, pow.indices.as_slice());

    let expected_pow_nonce = &[0xad, 0x63, 0x64, 0x76, 0xf7, 0x11, 0x74, 0x00];
    assert_eq!(expected_pow_nonce, pow.nonce.as_slice());

    let expected_difficulty = 409_392_556u32;
    assert_eq!(expected_difficulty, pow.difficulty);
}
...
fn test_block_header_hash() {
    ...
    assert_eq!(expected_block_header_hash, block_header_hash.as_bytes());
    //println!("Calculated block header hash: {:X?}", block_header_hash.as_bytes());

    let block_header_hash_for_pow = block_header.get_hash_for_pow();
    let expected_block_header_hash_for_pow = &[
    ...
    assert_eq!(expected_block_header_hash_for_pow, block_header_hash_for_pow.as_bytes());
}
